...
// Kiểm tra xem bảng avatar_frames có tồn tại không
$checkTable = $conn->query("SHOW TABLES LIKE 'avatar_frames'");
$avatarFramesExists = $checkTable && $checkTable->num_rows > 0;

// Kiểm tra xem cột avatar_frame_id có tồn tại không
$checkColumn = $conn->query("SHOW COLUMNS FROM users LIKE 'avatar_frame_id'");
$avatarFrameIdExists = $checkColumn && $checkColumn->num_rows > 0;

// Kiểm tra xem bảng user_avatar_frames có tồn tại không
$checkUserFrames = $conn->query("SHOW TABLES LIKE 'user_avatar_frames'");
$userAvatarFramesExists = $checkUserFrames && $checkUserFrames->num_rows > 0;

// Lấy thông báo từ session (nếu có)
$message = $_SESSION['khungavatar_message'] ?? "";
unset($_SESSION['khungavatar_message']);

// Cập nhật lựa chọn khung avatar
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['avatar_frame_id']) && $avatarFrameIdExists) {
    $frameId = intval($_POST['avatar_frame_id']);
    $stmt = $conn->prepare("UPDATE users SET avatar_frame_id = ? WHERE Iduser = ?");
    if ($stmt) {
        $stmt->bind_param("ii", $frameId, $userId);
        if ($stmt->execute()) {
            $_SESSION['khungavatar_message'] = "✅ Khung avatar đã được cập nhật!";
        } else {
            $_SESSION['khungavatar_message'] = "❌ Lỗi: " . $stmt->error;
        }
        $stmt->close();
    }
    // Redirect để tránh resubmit
    header("Location: khungavatar.php");
    exit();
}

// Lấy các khung avatar mà user đã sở hữu
$frames = [];
if ($avatarFramesExists) {
    if ($userAvatarFramesExists) {
        // Nếu có bảng user_avatar_frames, lấy khung đã mua và khung miễn phí
        $sql = "SELECT af.* FROM avatar_frames af 
                INNER JOIN user_avatar_frames uaf ON af.id = uaf.avatar_frame_id 
                WHERE uaf.user_id = ? 
                UNION 
                SELECT af.* FROM avatar_frames af 
                WHERE af.price = 0
        ...
        // Nếu không có bảng user_avatar_frames, chỉ lấy khung miễn phí
        $sql = "SELECT * FROM avatar_frames WHERE price = 0 ORDER BY id ASC";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->execute();
            $result = $stmt->get_result();
...
// Lấy khung avatar hiện tại của user
$currentFrameId = null;
if ($avatarFrameIdExists) {
    $stmt = $conn->prepare("SELECT avatar_frame_id FROM users WHERE Iduser = ?");
    if ($stmt) {
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($row = $result->fetch_assoc()) {
            $currentFrameId = $row['avatar_frame_id'];
        }
        $stmt->close();
    }
}
...
<body>
    <form method="post" id="frameForm">
        <h2>🖼️ Chọn khung avatar của bạn</h2>
        <input type="hidden" name="avatar_frame_id" id="avatar_frame_id">
        ...
        <button type="submit">💾 Lưu khung avatar</button>
    </form>
    ...
    <script>
        ...
        const hiddenInput = document.getElementById('avatar_frame_id');
