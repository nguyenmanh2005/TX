...
if ($checkLogs && $checkLogs->num_rows > 0) {
    $today = date('Y-m-d');
    $checkSpinStmt = $conn->prepare("SELECT * FROM lucky_wheel_logs WHERE user_id = ? AND spin_date = ?");
    $checkSpinStmt->bind_param("is", $userId, $today);
    $checkSpinStmt->execute();
    $hasSpunToday = $checkSpinStmt->get_result();
    $checkSpinStmt->close();
    if ($hasSpunToday && $hasSpunToday->num_rows > 0) {
        ...
        echo "<p>Lần quay cuối: {$lastSpin['reward_name']} - {$lastSpin['reward_value']} gtlm</p>";
    } else {
        echo "<p style='color: green;'>✅ User chưa quay wheel hôm nay</p>";
    }

    // Lịch sử 10 lần gần nhất
    $historyStmt = $conn->prepare("SELECT * FROM lucky_wheel_logs WHERE user_id = ? ORDER BY spun_at DESC LIMIT 10");
    ...
    if ($history && $history->num_rows > 0) {
        ...
        while ($item = $history->fetch_assoc()) {
            ...
            echo "<td>{$item['reward_value']} gtlm</td>";
            echo "</tr>";
        }
        echo "</table>";
    } else {
