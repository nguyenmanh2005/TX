...
if ($stmt) {
    ...
    if (!isset($user['current_theme_id']))
        $user['current_theme_id'] = null;
    if (!isset($user['current_cursor_id']))
        $user['current_cursor_id'] = null;
    if (!isset($user['chat_frame_id']))
        $user['chat_frame_id'] = null;
    if (!isset($user['avatar_frame_id']))
...
if (
    $checkThemesTable && $checkThemesTable->num_rows > 0 &&
...
if (
    $checkCursorsTable && $checkCursorsTable->num_rows > 0 &&
...
if (
    $checkChatFramesTable && $checkChatFramesTable->num_rows > 0 &&
...
if (
    $checkAvatarFramesTable && $checkAvatarFramesTable->num_rows > 0 &&
                    ...
                    <?php foreach ($themes as $theme): ?>
                        <div class="item-card <?= ($user['current_theme_id'] == $theme['id']) ? 'active' : '' ?>">
                            ...
                            <form method="POST" style="margin: 0;">
                                ...
                                <button type="submit" name="activate_theme" class="activate-button"
                                    <?= ($user['current_theme_id'] == $theme['id']) ? 'disabled' : '' ?>>
                    ...
                    <?php foreach ($cursors as $cursor): ?>
                        <div class="item-card <?= ($user['current_cursor_id'] == $cursor['id']) ? 'active' : '' ?>">
                            ...
                            <img src="<?= htmlspecialchars($cursor['cursor_image']) ?>"
                                alt="<?= htmlspecialchars($cursor['name']) ?>" class="item-image">
                            ...
                            <form method="POST" style="margin: 0;">
                                ...
                                <button type="submit" name="activate_cursor" class="activate-button"
                                    <?= ($user['current_cursor_id'] == $cursor['id']) ? 'disabled' : '' ?>>
                        ...
                        <div class="item-card <?= ($user['chat_frame_id'] == $frame['id']) ? 'active' : '' ?>">
                            ...
                            <?php if (!empty($frame['ImageURL'])): ?>
                                <img src="<?= htmlspecialchars($frame['ImageURL']) ?>"
                                    alt="<?= htmlspecialchars($frame['frame_name']) ?>" class="item-image">
                            <?php else: ?>
                            ...
                            <form method="POST" style="margin: 0;">
                                ...
                                <button type="submit" name="activate_chat_frame" class="activate-button"
                                    <?= ($user['chat_frame_id'] == $frame['id']) ? 'disabled' : '' ?>>
                    ...
                    <?php foreach ($avatarFrames as $frame): ?>
                        <div class="item-card <?= ($user['avatar_frame_id'] == $frame['id']) ? 'active' : '' ?>">
                            ...
                            <form method="POST" style="margin: 0;">
                                ...
                                <button type="submit" name="activate_avatar_frame" class="activate-button"
                                    <?= ($user['avatar_frame_id'] == $frame['id']) ? 'disabled' : '' ?>>
