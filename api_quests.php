...
elseif ($action === 'claim_reward') {
    ...
    if ($quest['reward_item_type'] && $quest['reward_item_id']) {
        ...
        if (isset($itemTableMap[$quest['reward_item_type']])) {
            $tableName = $itemTableMap[$quest['reward_item_type']];
            $itemIdColumn = $quest['reward_item_type'] === 'theme' ? 'theme_id' : 
                           ($quest['reward_item_type'] === 'cursor' ? 'cursor_id' : 
                           ($quest['reward_item_type'] === 'chat_frame' ? 'chat_frame_id' : 'avatar_frame_id'));
            
            // Kiểm tra xem người dùng đã có item này không
            $checkItemStmt = $conn->prepare("SELECT * FROM $tableName WHERE user_id = ? AND $itemIdColumn = ?");
            $checkItemStmt->bind_param("ii", $userId, $quest['reward_item_id']);
            $checkItemStmt->execute();
            $checkItemResult = $checkItemStmt->get_result();
            
            if ($checkItemResult->num_rows === 0) {
                // Thêm item cho người dùng
                $addItemStmt = $conn->prepare("INSERT INTO $tableName (user_id, $itemIdColumn) VALUES (?, ?)");
                $addItemStmt->bind_param("ii", $userId, $quest['reward_item_id']);
                $addItemStmt->execute();
                $addItemStmt->close();
            }
            $checkItemStmt->close();
