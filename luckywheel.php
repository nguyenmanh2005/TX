...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === 'spin_wheel') {
    ...
    if ($cuoc > $soDu || $cuoc <= 0) {
        $_SESSION['luckywheel_message'] = "⚠️ Số gtlm cược không hợp lệ!";
        ...
        if ($multi > 0) {
            ...
            $soDu += $thang;
            $_SESSION['luckywheel_message'] = "🎉 " . $ketQua . "! Bạn thắng " . number_format($thang) . " gtlm!";
            $_SESSION['luckywheel_class'] = "thang";
            $_SESSION['luckywheel_win'] = true;
        } else {
            $soDu -= $cuoc;
            $_SESSION['luckywheel_message'] = "😢 " . $ketQua . "! Mất " . number_format($cuoc) . " gtlm!";
            $_SESSION['luckywheel_class'] = "thua";
            $_SESSION['luckywheel_win'] = false;
        }

        if ($thang >= 5000000) {
            $message = "🎉 " . htmlspecialchars($tenNguoiChoi) . " vừa thắng lớn " . number_format($thang, 0, ',', '.') . " gtlm trong Vòng Quay May Mắn! 🎊";
    ...
    <style>
        ...
        * {
        ...
        @keyframes spinWheel {
            ...
            100% {
                transform: rotate(<?= $ketQua ? (360 * 5 + (array_search($ketQua, array_column($segments, 'label')) * 45)) : 0 ?>deg);
            }
        }
    ...
    <div class="game-box game-container">
        ...
        <form method="post" id="gameForm">
            ...
            <label style="display: block; margin: 15px 0 8px; font-weight: 600; color: white; font-size: 16px;">💰 Số
                gtlm cược:</label>
            <input type="number" name="cuoc" id="cuocInput" placeholder="Nhập số gtlm cược" required min="1"
                value=""><br>
        ...
        <?php if ($thongBao): ?>
            <div
                class="thongbao <?= $ketQuaClass ?> result-banner <?= $ketQuaClass === 'thang' ? 'result-win' : 'result-lose shake-on-lose' ?>">
