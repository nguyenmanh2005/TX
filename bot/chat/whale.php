<?php
return [
    'greet' => [
        "Hôm nay GTLM hơi nhiều, tìm chỗ xả bớt! 💰",
        "Ai cần lì xì không? Đại gia đang vui!",
        "Chào cả nhà, hôm nay cược lớn húp lớn nhé!",
        "Vừa húp Jackpot bên Mega Spin xong, qua đây quẩy tiếp!",
        " Gtlm bạc là phù du, quan trọng là anh em vui vẻ!"
    ],
    'win' => [
        "Có thế chứ! Húp nhẹ vài triệu GTLM.",
        " Gtlm chỉ là con số, quan trọng là cảm giác húp!",
        "Đại gia không bao giờ thua, chỉ là chưa thắng đủ lớn thôi!",
        "Thắng trận này xong tí đi bao anh em Bang mình nhậu!",
        "Đỉnh cao của sự giàu sang là húp liên tục! 😎"
    ],
    'lose' => [
        "Bay màu vài trăm nghìn không thành vấn đề.",
        "Coi như làm từ thiện cho anh em!",
        "Vận may tạm nghỉ thôi, trận sau All-in!",
        "Mất vài đồng lẻ, tí nạp thêm vài chục triệu GTLM lại húp ngay.",
        "Hôm nay server hơi đen với tui rồi đấy."
    ],
    'trash_talk' => [
        "Cược tí xíu thế bao giờ mới giàu được hả bạn?",
        "Nhìn cách bạn ra chiêu mà tui thấy tội nghiệp quá!",
        "Đại gia cược một ván bằng bạn chơi cả tháng rồi đó! 😂"
    ]
];
?>
