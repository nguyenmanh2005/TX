...
if ($action === 'guess' && !$gameOver) {
    if ($cuoc <= 0 || $cuoc > $soDu || !in_array($guess, ['red', 'black'])) {
        $_SESSION['ketqua'] = "Số gtlm cược hoặc lựa chọn màu không hợp lệ!";
        ...
        if ($card['color'] === $guess) {
            $soDu += $cuoc * 2;
            $_SESSION['ketqua'] = "Chúc mừng! Đúng màu {$card['color']}! Thắng " . number_format($cuoc * 2) . " gtlm";
            $_SESSION['ketquaShort'] = "Thắng";
            $_SESSION['ketqua_class'] = "bg-green-500 text-white animate-bounce";
        } else {
            $soDu -= $cuoc;
            $_SESSION['ketqua'] = "Sai rồi! Màu {$card['color']}. Mất " . number_format($cuoc) . " gtlm";
        ...
        if (!$stmt->execute()) {
            die("Lỗi: Không thể cập nhật Số Gtlm.");
        }
        $stmt->close();
    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="flex flex-col lg:flex-row gap-6">
                    ...
                    <table class="table-auto w-full text-left border-collapse border border-gray-300">
                        ...
                        <tbody>
                            <?php if (count($lichSu) === 0): ?>
                                <tr>
                                    <td colspan="8" class="text-center p-4">Chưa có lịch sử chơi</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($lichSu as $lich): ?>
                                    <tr>
                                        ...
                                        <td class="border border-gray-300 px-3 py-1">
                                            <?= $lich['Guess'] == 'red' ? 'Đỏ' : 'Đen' ?>
                                        </td>
                                        <td class="border border-gray-300 px-3 py-1">
                                            <?= $lich['CardColor'] == 'red' ? 'Đỏ' : 'Đen' ?>
                                        </td>
                                        <td class="border border-gray-300 px-3 py-1">
                                            <?= htmlspecialchars($lich['CardDisplay']) ?>
                                        </td>
            ...
            <div class="game-container lg:w-1/3">
                <h1 class="text-3xl font-bold mb-6 text-center" style="color: var(--primary-color);">🎴 Đoán Màu Bài
                </h1>
                <h2 class="text-xl mb-2 text-center" style="color: var(--text-dark);">Xin chào,
                    <strong><?= htmlspecialchars($tenNguoiChoi, ENT_QUOTES, 'UTF-8') ?></strong>
                </h2>
                <div class="balance-display text-center">💰 Số Gtlm: <strong><?= number_format($soDu, 0, ',', '.') ?>
                        gtlm</strong></div>

                <!-- Hiển thị lá bài -->
                <div class="flex justify-center mb-6">
                    <?php if ($card && $gameOver): ?>
                        <div class="card <?php if ($gameOver)
                            echo 'flipped'; ?>"
                            style="background-image: url('images/<?= $card['color'] == 'red' ? 'do.png' : 'den.png' ?>');">
                            <div class="card-front">
                                <div class="card-top-left"><?= htmlspecialchars($card['display'], ENT_QUOTES, 'UTF-8') ?>
                                </div>
                                <div class="card-center">
                                    <?= htmlspecialchars($card['display'][strlen($card['display']) - 1], ENT_QUOTES, 'UTF-8') ?>
                                </div>
                                <div class="card-bottom-right">
                                    <?= htmlspecialchars($card['display'], ENT_QUOTES, 'UTF-8') ?>
                                </div>
                            </div>
                            <div class="card-back">
                                <div class="card-top-left"><?= htmlspecialchars($card['display'], ENT_QUOTES, 'UTF-8') ?>
                ...
                <form method="POST" class="max-w-md mx-auto">
                    <div class="mb-4">
                        <input type="number" name="cuoc" placeholder="Nhập số gtlm cược"
                            class="w-full px-4 py-2 border rounded bg-white text-black" required min="1"
                            max="<?= $soDu ?>" value="<?= isset($_SESSION['cuoc']) ? $_SESSION['cuoc'] : '' ?>">
                    </div>
                    <div class="flex justify-center gap-6 mb-4">
                        <button type="submit" name="guess" value="red"
                            class="bg-red-600 text-white px-6 py-2 rounded hover:bg-red-700 flex items-center"
                            <?= $gameOver ? 'disabled' : '' ?>>
                            <span class="mr-2">♥♦</span> Đỏ
                        </button>
                        <button type="submit" name="guess" value="black"
                            class="bg-gray-800 text-white px-6 py-2 rounded hover:bg-gray-900 flex items-center"
                            <?= $gameOver ? 'disabled' : '' ?>>
                ...
                <?php if ($gameOver): ?>
                    <form method="POST" class="mt-4 text-center">
                        <button type="submit" name="reset_game"
                            class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">🎮 Chơi lại</button>
                    </form>
                <?php endif; ?>
