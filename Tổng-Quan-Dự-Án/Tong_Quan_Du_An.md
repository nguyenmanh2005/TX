...
*   **Role 0 (Thường dân/Dân chơi)**:
    *   Sở hữu tài khoản cá nhân, cày cuốc GTLM (Đơn vị tiền tệ chính).
...
*   **A. Phân cấp Tài sản (Wealth Tiers)**:
    ...
    *   `Commoners (Dân cày)`: Chơi an toàn, vốn nhỏ. Thường chơi Xí ngầu, Bầu cua, tích tiểu thành đại.
...
*   **C. Trí Tuệ Cảm Xúc & Ký Ức (Mood & Rivalry System)**:
    *   `Mood (Tâm trạng)`: Cập nhật liên tục. Nếu bot thua liên tiếp 5 ván, trạng thái chuyển sang "Cay cú" (Tilted) hoặc "Trầm cảm" (Depressed), dẫn đến việc cược liều mạng hơn. Nếu thắng, trạng thái là "Excited" (Hưng phấn).
...
*   **Tiền tệ kép**: `GTLM` dùng để cược và mua sắm cơ bản. `Xu Sự Kiện` chỉ xuất hiện và có giá trị trong mùa event hiện tại.
*   **Cửa hàng Tùy chỉnh (Vanity Shop)**: Nơi "đốt tiền" của đại gia. Mua các vật phẩm trang trí như: `Danh hiệu (Titles)` hiển thị trên đầu, `Khung Avatar (Avatar Frames)`, `Khung Chat` rực rỡ, `Giao diện nền (Themes)` và `Con trỏ chuột (Cursors)`.
...
*   **Lời Tiên Tri (Oracle Prophecy)**: Một dạng "Đảo Luật chơi". Đầu tuần, NPC Tiên Tri đưa ra 3 lời sấm truyền (VD: "Tuần sau XP x2" hoặc "Tuần sau Jackpot dễ nổ hơn"). Toàn server cùng bỏ tiền Vote. Lời tiên tri nào thắng sẽ thành hiện thực.
...
5.  **Rút Thăm (Raffle)**: Bỏ tiền mua Ticket chờ quay số cuối tuần.
...
7.  **Dân Gian & Giải Trí Khác**: Bầu Cua (Cyber Pets), Xóc Đĩa (Quantum Pulse), Oẳn Tù Tì (RPS), Tung Đồng Xu (Coinflip), Đoán Số, Cào Thẻ, Đoán Màu Bài.
...
1.  **Bot Kiểm Toán Chuyên Sâu (`tester_bot.php`)**:
    ...
    *   **Anti-Negative Balance**: Quét và ngăn chặn lỗi người chơi nhập tiền cược là số âm (`-5000`) hoặc số thập phân.
...
2.  **Bảo vệ Giao dịch (Database Transactions)**:
    *   Tất cả giao dịch chuyển tiền (Chợ, PvP, Trúng thưởng) đều được bao bọc bởi cấu trúc `BEGIN TRANSACTION` và `COMMIT` của MySQL, đảm bảo không bao giờ bị nhân đôi tiền khi mạng lag (Race conditions).
