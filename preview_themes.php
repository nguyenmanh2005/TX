        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($themeStmt) {
                ...
                if ($theme && $user['Money'] >= $theme['price']) {
                    ...
                    $_SESSION['preview_message'] = "❌ Không đủ gtlm hoặc theme không tồn tại!";
...
<body>
    <div class="balance-display">
        💰 Số Gtlm: <?= number_format($user['Money'], 0, ',', '.') ?> gtlm
    </div>
    ...
    <div class="theme-list">
        ...
        <?php foreach ($themes as $index => $theme): ?>
            ...
            ?>
            <div class="theme-item <?= $index === 0 ? 'active' : '' ?>" data-theme-id="<?= $theme['id'] ?>"
                data-index="<?= $index ?>" onclick="showTheme(<?= $index ?>)"
    ...
    <div class="theme-preview-container">
        <?php foreach ($themes as $index => $theme): ?>
            ...
            ?>
            <div class="theme-preview <?= $index === 0 ? 'active' : 'hidden' ?>" id="theme-<?= $index ?>"
                data-theme-index="<?= $index ?>" data-theme-name="<?= htmlspecialchars($theme['name'] ?? '') ?>"
                data-particle-count="<?= $particleCount ?>" data-particle-size="<?= $particleSize ?>"
                data-particle-color="<?= htmlspecialchars($particleColor) ?>"
                data-particle-opacity="<?= $particleOpacity ?>" data-shape-count="<?= $shapeCount ?>"
            ...
            <?php if (!empty($themes)): ?>
                ...
                <?php else: ?>
                    <form method="POST" style="display: inline;">
                        ...
                        <button type="submit" name="apply_theme" class="btn-buy" <?= !$canApply ? 'disabled' : '' ?>>
                            💰 Mua Và Áp Dụng (<?= number_format($currentTheme['price'], 0, ',', '.') ?> gtlm)
                        </button>
                    </form>
                <?php endif; ?>
            <?php endif; ?>
                ...
                actionsDiv.innerHTML = `
                    ...
                    <form method="POST" style="display: inline;">
                        ...
                        <button type="submit" name="apply_theme" class="btn-buy" ${!canBuy ? 'disabled' : ''}>
                            💰 Mua Và Áp Dụng (${parseInt(theme.price).toLocaleString('vi-VN')} gtlm)
                        </button>
                    </form>
                `;
            }
