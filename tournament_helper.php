...
function logTournamentGame($conn, $userId, $gameName, $betAmount, $winAmount, $isWin)
{
    ...
    while ($tournament = $result->fetch_assoc()) {
        ...
        // FIX PHP1401: gán biểu thức vào biến trước khi bind_param
        $isWinInt = $isWin ? 1 : 0;
        ...
        try {
            ...
            $insertStmt->bind_param(
                ...
                $betAmount,
                $winAmount,
                $isWinInt,       // FIX line 60: dùng biến thay vì $isWin ? 1 : 0
                $scorePoints
            );
            $insertStmt->execute();
            $insertStmt->close();
            ...
            $updateStmt->bind_param(
                "iddddii",
                $isWinInt,       // FIX line 77: dùng biến thay vì $isWin ? 1 : 0
                $betAmount,
                $winAmount,
                $scorePoints,
                $tournament['id'],
...
function calculateTournamentScore($betAmount, $winAmount, $isWin)
{
    if ($isWin) {
        ...
        // Thua: điểm = gtlm cược * 0.01 (khuyến khích chơi)
