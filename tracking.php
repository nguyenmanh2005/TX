...
function track_visit($conn) {
    ...
    foreach ($ignored_patterns as $pattern) {
        if (strpos($page_url, $pattern) !== false) return;
    }

    $ip_address = $_SERVER['REMOTE_ADDR'];
    
    // Exclude specific IP (e.g., admin's IP)
    ...
    if ($referrer !== 'Direct') {
        ...
        if ($ref_host) {
            if (strpos($ref_host, 'google') !== false) $source = 'Google';
            elseif (strpos($ref_host, 'facebook') !== false) $source = 'Facebook';
            elseif (strpos($ref_host, 'youtube') !== false) $source = 'YouTube';
            elseif (strpos($ref_host, 'twitter') !== false || strpos($ref_host, 't.co') !== false) $source = 'Twitter';
            elseif (strpos($ref_host, $_SERVER['HTTP_HOST']) !== false) $source = 'Internal';
