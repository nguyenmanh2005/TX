...
switch ($action) {
    case 'create_challenge':
        ...
        if ($betAmount <= 0) {
            echo json_encode(['success' => false, 'message' => 'Số gtlm cược phải lớn hơn 0!']);
            exit;
        }
        ...
        if ($userBalance['Money'] < $betAmount) {
            echo json_encode(['success' => false, 'message' => 'Bạn không đủ gtlm để tạo challenge!']);
            exit;
        }

        if ($opponent['Money'] < $betAmount) {
            echo json_encode(['success' => false, 'message' => 'Đối thủ không đủ gtlm để chấp nhận challenge!']);
            exit;
        }

        // Trừ gtlm của challenger ngay
        ...
        try {
            ...
            createNotification(
                ...
                'friend_request',
                'Thách Đấu Mới!',
                htmlspecialchars($userBalance['Name'] ?? 'Ai đó') . " đã thách đấu bạn " . number_format($betAmount, 0, ',', '.') . " gtlm!",
                '⚔️',
                'pvp_challenge.php?id=' . $challengeId,
                $challengeId,
                true
            );
    ...
    case 'accept_challenge':
        ...
        if ($userBalance['Money'] < $challenge['bet_amount']) {
            echo json_encode(['success' => false, 'message' => 'Bạn không đủ gtlm để chấp nhận challenge!']);
            exit;
        }
    ...
    case 'submit_choice':
        ...
        if ($current['challenger_choice'] && $current['opponent_choice']) {
            ...
            // Cập nhật kết quả và trả gtlm
            $conn->begin_transaction();
            try {
                $totalBet = $challenge['bet_amount'] * 2;

                if ($winnerId) {
                    // Trả gtlm cho người thắng
                ...
                $insertHistory->bind_param(
                    ...
                    $current['challenger_choice'],
                    $current['opponent_choice'],
                    $result,
                    $winnerId
                );
    ...
    switch ($gameType) {
        case 'coinflip':
            ...
            if ($challengerChoice == $opponentChoice) {
                ...
                if ($opponentChoice == $randomResult)
                    return 'opponent_win';
            }
            break;
