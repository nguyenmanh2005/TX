...
<body>
    ...
    // Chart.js for hopmu game
    const ctxHopmu = document.getElementById('gameChart');
    if (ctxHopmu) {
        const gameChart = new Chart(ctxHopmu.getContext('2d'), {
            type: 'doughnut',
            data: {
    ...
    window.addEventListener('load', loadHopmuHistory);
    ...
    // Improved history loading function
    async function loadHopmuHistory() {
        try {
            const response = await fetch('hopmu.php?action=get_history', {
                headers: {
