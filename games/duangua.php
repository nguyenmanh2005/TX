...
<body>
    ...
    // Chart.js for duangua game
    const ctxDuangua = document.getElementById('gameChart');
    if (ctxDuangua) {
        const gameChart = new Chart(ctxDuangua.getContext('2d'), {
            type: 'doughnut',
            data: {
    ...
    window.addEventListener('load', loadDuanguaHistory);
    ...
    // Improved history loading function
    async function loadDuanguaHistory() {
        try {
            const response = await fetch('duangua.php?action=get_history', {
                headers: {
