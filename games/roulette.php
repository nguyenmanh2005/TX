...
<body>
    ...
    // Load game history for roulette
    ...
    // Chart.js for roulette game
    const ctxRoulette = document.getElementById('gameChart');
    if (ctxRoulette) {
        const gameChart = new Chart(ctxRoulette.getContext('2d'), {
            type: 'doughnut',
            data: {
    ...
    window.addEventListener('load', loadRouletteHistory);
    ...
    async function loadRouletteHistory() {
        try {
            const response = await fetch('roulette.php?action=get_history', {
                headers: {
