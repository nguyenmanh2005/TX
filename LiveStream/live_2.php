...
$botUser = getOrCreateBotStreamerUser($conn, 'bot_2', 50000000);
...
if (!isset($botUserId)) { header('Location: ../login.php'); exit; }
...
$gameThang = 0;
$gameThua = 0;
$sqlStats = "SELECT COUNT(*) as total, SUM(CASE WHEN WinAmount > 0 THEN 1 ELSE 0 END) as wins FROM history_xocdia WHERE Iduser = ?";
...
<body>
    ...
    <script>
        ...
        function triggerPulse() {
            ...
            $.post('?action=play', { bets: JSON.stringify(chargesData) }, function(res) {
                if (res.success) {
                    setTimeout(() => {
    ...
    <script>
        async function loadXocdiaHistory() {
            try {
                const response = await fetch('../game_history_universal.php?action=get_history&game=xocdia');
        ...
        (function () {
            window.themeConfig = {
                ...
                particleOpacity: 0.5,
                shapeCount: 15,
                shapeColors: ["#00f2fe", "#712cf9", "#ffffff"],
                shapeOpacity: 0.2,
                bgGradient: ["#000000", "#0a0a2a", "#1a0033"]
            };
            const prefix = '../';
            ['threejs-background.js', 'assets/js/game-effects.js', 'assets/js/game-effects-auto.js'].forEach(src => {
...
<script src="bots/bot_2.js"></script>
