...
$botUser = getOrCreateBotStreamerUser($conn, 'bot_48', 50000000);
    ...
    <script>
                    setTimeout(() => {
                        ...
                        if (data.rewardAmount > 0) {
                            ...
                            showResultBadge(true, data.rewardAmount, data.message);
                            if (window.GameEffects) {
                                GameEffects.showWin(data.rewardAmount);
                                if (typeof GameEffects.coinBlast === 'function') {
                                    const cx = (event && event.clientX) ? event.clientX : window.innerWidth / 2;
                                    const cy = (event && event.clientY) ? event.clientY : window.innerHeight / 2;
                                    GameEffects.coinBlast(cx, cy, 30);
                                }
                            }
                            ...
                            showResultBadge(false, 0, data.message);
                            if (window.GameEffects) {
                                GameEffects.showLoss(50000);
                            }
                        }
...
<script src="bots/bot_48.js?v=<?= time() ?>"></script>
